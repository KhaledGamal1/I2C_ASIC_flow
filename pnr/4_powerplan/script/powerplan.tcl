...
# Creation VDD and VSS nets for Network (already exist in i2c rtl)

# --- Ensure VDD and VSS exist and are explicitly typed as Power/Ground
if {[sizeof_collection [get_nets -quiet VDD]] == 0} { create_net -power VDD }
if {[sizeof_collection [get_nets -quiet VSS]] == 0} { create_net -ground VSS }

# --- Force net_type attributes (Fixes PGR-599)
set_attribute [get_nets VDD] net_type power
set_attribute [get_nets VSS] net_type ground

# --- Option A: Run automatic PG net connection across the design
